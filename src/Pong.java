...
/*
This is a basic implementation of the game Pong.
The ball is randomly fired to one side, which is determined by a pseudorandom number generator.
If the ball hits the left wall, the right player scores a point and vice versa. After the point,
the ball is fired toward the winner of the rally.

The key "r" moves the left paddle up and "v" moves it down.
"U" moves the right paddle up and "n" moves it down.

If the ball hits the topmost edge of the paddle, it travels at -75 degrees upwards and downwards for the bottom edge of the paddle.
If it hits somewhere in between the middle and edge of the paddle, it travels between 0 to 75 degrees based on the exact point of contact.

For this implementation, there is a speedup method in the World class that gradually increases the ball's speed according to the sign of its velocity.

 */


/*
The Pair class has two fields. This makes it easier to access the x and y values of velocity and position that are used in the game.
 */
class Pair{
    public double x;
    public double y;
...
/*
The Ball class is for the ball that is manipulated in the game.
The variables include, position, velocity, acceleration, radius, and dampening.
 */
...
/*
The Paddle class is for the paddles that manipulate the movement of the ball in the game.
 */
...
/*
The World class initializes the ball and paddles that are used in the game.
 */
class World {
    int height;
    int width;
...
/*
The Pong class runs the actual game and initializes the World.
 */
